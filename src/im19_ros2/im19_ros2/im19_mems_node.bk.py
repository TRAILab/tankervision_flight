...
class IM19MemsNode(Node):
    def __init__(self):
        ...
        self._output_dir_base = self.output_dir
        self.bin_file = None
    ...
    def _ensure_output_file(self):
        ...
        self.bin_path = os.path.join(self.output_dir, f"im19_mems_raw_{time.strftime('%Y%m%d_%H%M%S', now)}.bin")
        self.bin_file = open(self.bin_path, "ab")
        self.get_logger().info(f"IMU output: {self.bin_path}")
